...
const Species = () => {
  ...
  const getTotalSpecies = async (page) => {
    setLoading(true)
    try {
      const res = await api.get(`${appUrls?.SPECIES_URL}/?page=${page}`);
  ...
  return (
    <div className="w-full mt-[108px] mb-[46px] flex flex-col">
      ...
      <div className="flex flex-col gap-[31px]">
        <p className="text-NEUTRAL-900 text-base leading-6 font-inter">Species</p>
          ...
          <table className="min-w-full border border-[#A4A7B766] ">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-6 border-[#0D2FA112] border-b text-NEUTRAL-900 font-inter text-base leading-6 font-medium whitespace-nowrap text-left">Classification</th>
                <th className="px-4 py-6 border-[#0D2FA112] border-b text-NEUTRAL-900 font-inter text-base leading-6 font-medium whitespace-nowrap text-left">Eye colors</th>
              ...
              {loading ?
                ...
                species?.length > 0 ?
                  species?.map((specie, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate("/species/details", { state: specie.url })}>
                      <td className="px-4 py-[20px] border-b border-[#0D2FA112]">
                        <input type="checkbox" />
                      </td>
                      <td className="px-4 py-[20px] font-inter text-NEUTRAL-700 text-base leading-6 border-b border-[#0D2FA112]">{specie.name}</td>
                      <td className="px-4 py-[20px] font-inter text-NEUTRAL-700 capitalize leading-6 text-base border-b border-[#0D2FA112]">{specie.classification}</td>
                      <td className="px-4 py-[20px] font-inter text-NEUTRAL-700 text-base leading-6 border-b capitalize border-[#0D2FA112]">{specie.eye_colors}</td>
                      <td className="px-4 py-[20px] font-inter text-NEUTRAL-700 text-base leading-6 border-b capitalize border-[#0D2FA112]">{specie.hair_colors}</td>
                      <td className="px-4 py-[20px] whitespace-nowrap font-inter text-NEUTRAL-700 leading-6 text-base border-b border-[#0D2FA112]">{`${specie.average_height !== "n/a" ? `${specie.average_height} CM` :  specie.average_height} `}</td>
                      <td className="px-4 py-[20px] font-inter text-NEUTRAL-700 text-base leading-6 border-b border-[#0D2FA112]">
                        {new Date(specie.created).toLocaleDateString().slice(0, -2)}
                      </td>
                    </tr>
                  )) : (
                    <tr>
